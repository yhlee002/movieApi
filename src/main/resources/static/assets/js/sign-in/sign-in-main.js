...
$(function () {
    ...
    let oauthMsg = document.getElementById('oauthMsg').value; // "[[${oauthMsg}]]"

    if (oauthMsg === "kakao user") { // 카카오 로그인 api 구현시 사라질 조건
        window.alert("카카오 아이디를 통해 로그인이 필요한 회원입니다.");
    } else if (oauthMsg === "conventional user") {
        window.alert("비밀번호를 통한 로그인이 필요한 회원입니다.");
    } else if (oauthMsg === "not user") {
