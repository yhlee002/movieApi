...
public interface BoardImpRepository extends JpaRepository<BoardImp, Long> {
    ...
    @Query(value = "select b.* from board_imp b join member m on b.writer_no = m.mem_no where m.name like %:writerName%"
            , nativeQuery = true)
    //  order by b.id desc limit ?2, ?3
