...
public class UserDetail extends User {
    public UserDetail(Member member) {
        super(member.getIdentifier(), member.getPassword(), authorities(member));
    }
    ...
    private static Collection<? extends GrantedAuthority> authorities(Member member) {
        ...
        if (member.getRole().equals(MemberRole.ROLE_ADMIN)) {
            ...
            log.info("들어온 memeber({})의 권한 : ROLE_ADMIN", member.getIdentifier());
            ...
            log.info("들어온 memeber({})의 권한 : ROLE_USER", member.getIdentifier());
        }
        return authorities;
    }
}
